...
# Get STE Result
    async def async_get_ste_result(self):
        if self.device:
            async with BleakClient(self.device) as self.client:
                #Ste Result Read
                self.ste_results = await self.client.read_gatt_char(c.ServiceShortTermExperiment["STEResults"])
    ...
    async def __async__toggle_stream(self,on = True):
        if self.device:
            ...
            #Mode Selection Write
                ...
                await self.client.write_gatt_char(c.ServiceSCDSettings["SCDGenericCommands"],  b"\x20")
                return True
        self.is_connected = False
        raise BleakError(f"The device with address {self.ble_address} could not be connect.")
...
# Get Mode Status
    async def __async__get_mode_status(self):
        if self.device:
            async with BleakClient(self.device) as self.client:
            #Mode Selection Read
                mode_selection = await self.client.read_gatt_char(c.ServiceSCDSettings["ModeSelection"])
    ...
    async def __async__change_mode_status(self,on = True):
        if self.device:
            async with BleakClient(self.device) as self.client:
                ...
                #Mode Selection Write
                    write = 0 if on else 255
                    await self.client.write_gatt_char(c.ServiceSCDSettings["ModeSelection"],bytearray(
                [
...
# Get SCD110 information
    async def __async__get_scd_info(self):
        if self.device:
            ...
            #Device name Read
                devname = await self.client.read_gatt_char(c.ServiceGenericAccess["DeviceName"])
                print(" Device Name: {0}".format(devname))
            #System Id Read
                sysid = await self.client.read_gatt_char(c.ServiceDeviceInformation["ServiceID"])
                print(" System Id: {0}".format(sysid))
            #Serial Number Read
                serial = await self.client.read_gatt_char(c.ServiceDeviceInformation["SerialNumberString"])
                print(" Serial No: {0}".format(serial))
            #Firmware Revisin Number Read
                firm = await self.client.read_gatt_char(c.ServiceDeviceInformation["FirmwareRevisionString"])
                print(" Firmware No: {0}".format(firm))
            #Hardware Revision Number Read
                hard = await self.client.read_gatt_char(c.ServiceDeviceInformation["HardwareRevisionString"])
                print(" Hardware No: {0}".format(hard))                
            #Software Revision Number Read
                soft = await self.client.read_gatt_char(c.ServiceDeviceInformation["SoftwareRevisionString"])
                print(" Software No: {0}".format(soft))    
            #Manufacture Name Read
                manufact = await self.client.read_gatt_char(c.ServiceDeviceInformation["ManufacturerNameString"])
...
# Disconnect Handling Event
    async def show_disconnect_handling(self):
        if self.device:
            ...
            async with BleakClient(self.device, disconnected_callback=disconnected_callback) as self.client:
